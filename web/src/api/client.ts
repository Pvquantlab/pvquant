...
export const api = {
  ...
  /** v2.270: Hakkında — veri kaynakları ve lisanslar. */
  hakkinda: async (): Promise<Hakkinda | null> => {
    if (TABAN == null) return null;
    try { return await getir<Hakkinda>(`/v1/hakkinda`); } catch { return null; }
  },
  ...
  dengesizlik: async (p: string, gun = 90): Promise<Dengesizlik | null> => {
    if (TABAN == null) return null;
    try { return await getir<Dengesizlik>(`/v1/plants/${p}/dengesizlik?gun=${gun}`); } catch { return null; }
  },
  ...
  hijyen: async (p: string, gun = 30): Promise<Hijyen | null> => {
    if (TABAN == null) return null;
    try { return await getir<Hijyen>(`/v1/plants/${p}/hijyen?gun=${gun}`); } catch { return null; }
  },
